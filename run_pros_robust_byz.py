...
lambd_list = [lambd, lambd, lambd, lambd, lambd, lambd] #10, 0, 0.1
gamma_list = [args.gamma]*client_num
#number of slices
slice_num = np.array([521, 966, 288, 1216, 578, 318])
...
def update_global_model(net_server, net_clients, diff, epc, prev, client_weight=None, aggr='mean', mode='standard'):
    ...
    if aggr == 'mean':
        if mode == 'fedbn':
            ...
            '''select Byzantine client'''
            np.random.seed(epc)
            byz_idx = np.random.randint(0, client_num)
            print('Byzantine index:', byz_idx)
            ...
            for lyr, (key, _) in enumerate(net_server.named_parameters()):
                ...
                if 'bn' not in key:
                    ...
                    for i in range(client_num):
                        ...
                        if i == byz_idx:
                            grads = grads+torch.normal(0,0.05, diff[i][lyr].shape).to(device) #mean: 0, std: 0.05
        ...
        if mode == 'standard':
            ...
            for lyr, (key, _) in enumerate(net_server.named_parameters()):
                ...
                for i in range(client_num):
                    grads = diff[i][lyr]*client_weight[i]
                    if i == byz_idx:
                        grads = grads+torch.normal(0,0.05, diff[i][lyr].shape).to(device) #mean: 5, variance: 1
                    agg_grad.data.add_(grads)
    ...
    if aggr == 'gam_mean':
        if mode == 'fedbn':
            ...
            for lyr, (key, _) in enumerate(net_server.named_parameters()):
                if 'bn' not in key:
                    points = torch.vstack([diff[i][lyr].view(-1).unsqueeze(0)+torch.normal(0,0.05, diff[i][lyr].view(-1).unsqueeze(0).shape).to(device)
                                           if byz_idx==i else diff[i][lyr].view(-1).unsqueeze(0) for i in range(client_num)]).data.cpu().numpy()
                    '''gamma selection'''
                    if epc == 0:
                        gamma = args.gamma
                    else:
...
if __name__ == "__main__":
    ...
    if args.state == 'train':
        ...
        for epoch_num in tqdm(range(max_epoch), ncols=70):
            ...
            for client_idx in source_site_idx:
                ...
                if loss_fn == 'auto_gamma_dice_loss':
                    #wei_score = torch.vstack(score_clients[client_idx])
    ...
    if args.state =='test':
        print('-----Testing------')
        for epc in range(49, max_epoch, 1):
