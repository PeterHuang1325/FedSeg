            ...
            for lyr, (key, _) in enumerate(net_server.named_parameters()):
                ...
                if 'bn' not in key:
                    ...
                    for i in range(client_num):
                        ...
                        if i == byz_idx:
                            grads = grads+torch.normal(0,0.05, diff[i][lyr].shape).to(device) #mean: 0, variance: 0.05
        ...
        if mode == 'standard':
            ...
            for lyr, (key, _) in enumerate(net_server.named_parameters()):
                ...
                for i in range(client_num):
                    grads = diff[i][lyr]*client_weight[i]
                    if i == byz_idx:
                        grads = grads+torch.normal(0,0.05, diff[i][lyr].shape).to(device) #mean: 0, variance: 1e-3
                    agg_grad.data.add_(grads)
                #update global model
                net_server.state_dict()[key].data.add_(agg_grad)
                updates = net_server.state_dict()[key].data
